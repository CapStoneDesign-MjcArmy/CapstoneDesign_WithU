...
import 'package:withu/AboutKiosk/Certificate/id_check.dart'; // 다른 페이지의 Dart 파일 import

class Idinputfam extends StatefulWidget {
  final String initialValue;

  Idinputfam({required this.initialValue}); // 생성자 추가

  @override
  _HomeScreenState createState() => _HomeScreenState();
}

class _HomeScreenState extends State<Idinputfam> {
  ...
  void _onConfirmButtonPressed() {
    if (_controller.text.length == 13) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => Idcheck(idNumber: "가족관계증명서")), // 다음 페이지로 라우팅
      );
    } else {
      // 13자리 미만이면 알림창 표시
      showDialog(
                          ...
                          children: <Widget>[
                            ...List.generate(3, (rowIndex) {
                              return Row(
                                ...
                                children: List.generate(3, (colIndex) {
                                  ...
                                  return Padding(
                                    padding:
                                        const EdgeInsets.all(4.0), // 버튼 간격 조정
                                    child: ElevatedButton(
                                      onPressed: () => _onKeypadButtonPressed(
                                          number.toString()),
                                      child: Text(
                                        '$number',
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Padding(
                                  padding:
                                      const EdgeInsets.all(1.0), // 버튼 간격 조정
                                  child: ElevatedButton(
